...
class Home extends BaseController
{
    ...
    public function proses_login()
    {
        ...
        $data1 = $atmodel->where('nip_at', $username)
        ->join('pt','pt.idPT = at.idPT')
        ->join('kt','kt.idKT = at.idKT')
        ->first();

        $data2 = $ktmodel->where('nip_kt', $username)
        ->join('pj','pj.idPJ = kt.idPJ')
        ->join('pt','pt.idPT = kt.idPT')
        ->first();

        $data3 = $ptmodel->where('nip_pt', $username)
        ->join('pj','pj.idPJ = pt.idPJ')
        ->first();

        $data4 = $pjmodel->where('nip_pj', $username)->first();
